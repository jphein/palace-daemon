...
| **Clients** | `clients/palace-mode` — shell CLI that flips between local (in-process) and remote (daemon) palace modes by toggling `PALACE_DAEMON_URL` in `~/.claude/settings.local.json`. | The `install` subcommand re-applies plugin-cache customizations after a Claude Code plugin update; that path layout is JP-specific to the plugin install's cache directory. The mode-switching itself is universal; the install path needs to be parameterized or removed for the upstream version. | `clients/palace-mode` |
