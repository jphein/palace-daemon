...
| [#12](https://github.com/rboarescu/palace-daemon/pull/12) | OPEN, awaiting review | `fix(clients): remove embedded API key + URL defaults from palace-mode` — `clients/palace-mode` shipped with `DEFAULT_URL=http://disks.jphe.in:8085` and a real hex `DEFAULT_KEY` (rotated, but still in upstream's source). Reads both from env, fails fast in `remote` mode if either is unset. |
